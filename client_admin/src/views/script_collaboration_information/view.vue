	...
	<el-main class="bg edit_wrap">
		<el-form ref="form" :model="form" status-icon label-width="120px" v-if="is_view()">
				...
				<el-form-item label="剧本名称" prop="script_name">
							  ...
							  v-if="user_group === '管理员' || (form['script_collaboration_information_id'] && $check_field('set','script_name')) || (!form['script_collaboration_information_id'] && $check_field('add','script_name'))" :disabled="disabledObj['script_name_isDisabled']"></el-input>
				...
				<el-form-item label="剧本类型" prop="script_type">
							  ...
							  v-if="user_group === '管理员' || (form['script_collaboration_information_id'] && $check_field('set','script_type')) || (!form['script_collaboration_information_id'] && $check_field('add','script_type'))" :disabled="disabledObj['script_type_isDisabled']"></el-input>
				...
				<el-form-item label="剧本价格" prop="script_price">
						...
						v-if="user_group === '管理员' || (form['script_collaboration_information_id'] && $check_field('set','script_price')) || (!form['script_collaboration_information_id'] && $check_field('add','script_price'))" :disabled="disabledObj['script_price_isDisabled']"></el-input-number>
				...
				<el-form-item label="拼团用户" prop="group_user">
							...
							<!--v-if="user_group === '管理员' || (form['script_collaboration_information_id'] && $check_field('set','group_user')) || (!form['script_collaboration_information_id'] && $check_field('add','group_user'))" :disabled="disabledObj['group_user_isDisabled']"></el-input>-->
							<!--<div v-else-if="$check_field('get','group_user')">{{form['group_user']}}</div>-->
							<el-select v-if="user_group === '管理员' || (form['script_collaboration_information_id'] && $check_field('set','group_user')) || (!form['script_collaboration_information_id'] && $check_field('add','group_user'))" id="group_user" v-model="form['group_user']" :disabled="disabledObj['group_user_isDisabled']">
				...
				<el-form-item label="用户姓名" prop="user_name">
							  ...
							  v-if="user_group === '管理员' || (form['script_collaboration_information_id'] && $check_field('set','user_name')) || (!form['script_collaboration_information_id'] && $check_field('add','user_name'))" :disabled="disabledObj['user_name_isDisabled']"></el-input>
				...
				<el-form-item label="拼团数量" prop="number_of_groups">
						...
						v-if="user_group === '管理员' || (form['script_collaboration_information_id'] && $check_field('set','number_of_groups')) || (!form['script_collaboration_information_id'] && $check_field('add','number_of_groups'))">
				...
				<el-form-item label="拼团时间" prop="group_time">
								<el-date-picker :disabled="disabledObj['group_time_isDisabled']" v-if="user_group === '管理员' || (form['script_collaboration_information_id'] && $check_field('set','group_time')) || (!form['script_collaboration_information_id'] && $check_field('add','group_time'))" id="group_time"
	...
	export default {
		...
		data() {
			return {
				...
				url_add: "~/api/script_collaboration_information/add?",
				url_set: "~/api/script_collaboration_information/set?",
				url_get_obj: "~/api/script_collaboration_information/get_obj?",
				url_upload: "~/api/script_collaboration_information/upload?",

				query: {
					"script_collaboration_information_id": 0,
				},
				...
				form: {
								"script_name":  '', // 剧本名称
										...
										"group_time":  '', // 拼团时间
											"script_collaboration_information_id": 0, // ID
		...
		methods: {
          ...
          this.form["group_time"] = null;
			...
			is_view(){
				...
				if(!bl){
					bl = this.$check_action('/script_collaboration_information/table','add');
					console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
				}
				if(!bl){
					bl = this.$check_action('/script_collaboration_information/table','set');
					console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
				}
				if(!bl){
					bl = this.$check_action('/script_collaboration_information/view','add');
					console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
				}
				if(!bl){
					bl = this.$check_action('/script_collaboration_information/view','set');
					console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
				}
				if(!bl){
					bl = this.$check_action('/script_collaboration_information/view','get');
